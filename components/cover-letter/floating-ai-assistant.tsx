...
import { applyCoverLetterToolResult, type CoverLetterToolResult } from '@/lib/utils/apply-cover-letter-tool-results'
import AIAssistantChat from '@/components/ai-elements/ai-assistant-chat'

const QUICK_ACTIONS = [
  { label: 'Generate cover letter', prompt: 'Generate a cover letter based on my CV and the job details I\'ve provided' },
  { label: 'Make concise', prompt: 'Make my cover letter more concise and impactful' },
  { label: 'Formal tone', prompt: 'Rewrite my cover letter in a more formal tone' },
  { label: 'Stronger opening', prompt: 'Improve the opening paragraph to be more attention-grabbing' },
  { label: 'Add enthusiasm', prompt: 'Add more enthusiasm and genuine interest in the role' },
]

export default function CoverLetterFloatingAIAssistant() {
      ...
      for (const part of lastMessage.parts) {
        if (isToolUIPart(part)) {
          ...
          if (toolPart.state === 'output-available' && toolPart.output && !processedToolKeys.current.has(toolKey)) {
            ...
            const result = applyCoverLetterToolResult(toolPart.output)
    ...
    <>
      ...
      >
        <Button
          ...
          aria-label={isOpen ? 'Close cover letter assistant' : 'Open cover letter assistant'}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div role="dialog" aria-label="Cover letter assistant" className="max-w-4xl mx-auto px-4 pb-6 pointer-events-auto">
              <AIAssistantChat
                ...
                emptyStateText="Ask me to improve your cover letter, or try a quick action:"
                emptyStateNotProText="Upgrade to Pro to use AI-powered cover letter editing."
                placeholder="Ask me to update your cover letter... (⌘+Enter to send)"
                onClose={handleClose}
